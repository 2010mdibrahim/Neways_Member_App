...
class FoodMenuAPIService {
  ...
  static getAllMenu({required data}) async {
    final box = GetStorage();
    Dio _http = getDio();
    ...
    try {
      Response response = await _http.post('/food-menus', data: data);
      print(response.statusCode);
      if (response.statusCode == 200) {
        print(response.data['data']);
        return FoodMenuResponse.fromJson(response.data['data']);
      }
    } on DioError catch (e) {
      if (e.response?.statusCode == 401) {
        return {"error": "Server Error"};
